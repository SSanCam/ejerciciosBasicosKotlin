...
fun main() {
    ...
    val cadenaClientes =
        "nif;nombre;email;teléfono;descuento\n01234567L;Luis González;luisgonzalez@mail.com;656343576;12.5\n71476342J;Macarena Ramírez;macarena@mail.com;692839321;8\n63823376M;Juan José Martínez;juanjo@mail.com;664888233;5.2\n98376547F;Carmen Sánchez;carmen@mail.com;667677855;15.7".trimIndent()

    val lineas = cadenaClientes.split("\n")
    val campos = lineas[0].split(";")

    val clientes = mutableMapOf<String, MutableMap<String, Any>>()
    for (datos in 1..<lineas.size) {
        val valores = lineas[datos].split(";")
        val clienteInfo = mutableMapOf<String, Any>()

        for (i in campos.indices) {
            clienteInfo[campos[i]] = when (campos[i]) {
                "descuento" -> valores[i].toDouble()
                else -> valores[i]
            }
        }
        clientes[valores[0]] = clienteInfo
    }
    println(clientes)
}