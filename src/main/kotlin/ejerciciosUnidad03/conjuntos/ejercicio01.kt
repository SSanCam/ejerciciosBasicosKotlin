...
hecho más de una compra en el mes, por lo que la función debe retornar una estructura que contenga cada domicilio una sola vez.
*/

fun domiciliosFacturacion(compras: Map<String, Triple<Int, Double, String>>): Set<String> {
    return compras.values.map { it.third }.toSet()
}

fun main() {
    val compras = mapOf(
        "Nuria Costa" to Triple(5, 12780.78, "Calle Las Flores 355"),
        "Jorge Russo" to Triple(7, 699.0, "Mirasol 218"),
        "Nuria Costa" to Triple(7, 532.90, "Calle Las Flores 355"),
        "Julián Rodriguez" to Triple(12, 5715.99, "La Mancha 761"),
        "Jorge Russo" to Triple(15, 958.0, "Mirasol 218")
    )

    val domiciliosFacturacion = domiciliosFacturacion(compras)
    println("Domicilios de facturación:")
    for (domicilio in domiciliosFacturacion){
        println("-> $domicilio")
    }
}